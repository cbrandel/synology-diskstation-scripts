...
LOGFILE=/var/synolog/synosys.log
ret=0
...
then
  ...
  if [ "$ret" -eq "0" ]
  then
    printf "info\t%s\tSYSTEM:\tUpdated DNS from DHCP." "$ts" >> $LOGFILE
  else
    printf "err\t%s\tSYSTEM:\tError Updating DNS from DHCP. Code: [%s]." "$ts" "$ret" >> $LOGFILE
  fi
fi
exit $ret
